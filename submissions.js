...
function renderDashboard() {
  ...
  const highlightCards = [
    ...
    {
      title: '生活与财务',
      body: `${rangeLabel}记录收支 ${accountingItems.length} 笔，并为未来愿望净攒入 ${formatSavingsMoney(savingsNet)}。`,
      note: `记账结余 ${formatAccountingMoney(accountingIncome - accountingExpense)} · 攒钱存入 ${formatSavingsMoney(savingsDeposit)} · 取出 ${formatSavingsMoney(savingsWithdrawal)}`
    },
    {
      title: '心理与沟通',
  ...
  const coverageRows = [
    ...
    {
      label: '攒钱规划',
      ratio: savingsDays / Math.max(1, days),
      detail: `记录 ${savingsItems.length} 笔 · 净攒入 ${formatSavingsMoney(savingsNet, { compact:true })} · 目标 ${state.savings?.goals?.length || 0} 个`
    },
    {
      label: '健康管理',
