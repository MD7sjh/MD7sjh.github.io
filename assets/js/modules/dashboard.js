...
function renderDashboard(){
  ...
  const experiments=experimentRunsInRange(start,end), experimentCompleted=experiments.filter(i=>i.status==='completed').length, experimentMetrics=experiments.reduce((s,i)=>s+(i.metrics?.length||0),0);
  ...
  makeOrUpdateChart('experimentDashboardChart','experimentDashboard',{type:'bar',data:{labels:dates.map(d=>d.slice(5)),datasets:[{label:'实验记录',data:dates.map(d=>experimentRunsInRange(d,d).length),backgroundColor:'#43AA8B',borderRadius:10},{label:'完成实验',data:dates.map(d=>experimentRunsInRange(d,d).filter(i=>i.status==='completed').length),backgroundColor:'#9B5DE5',borderRadius:10}]},options:{responsive:true,maintainAspectRatio:false,scales:{y:{beginAtZero:true,ticks:{precision:0}}}}});
