...
(() => {
  ...
  const SAVE_DELAY_MS = Number(config.saveDelayMs) || 900;

  if (!SUPABASE_URL || !SUPABASE_PUBLISHABLE_KEY) {
    console.error('[Cloud Sync] Supabase public configuration is missing.');
    alert('Supabase 配置缺失，请检查 assets/js/config/supabase.config.js。');
    return;
  }
  ...
  function injectUi() {
    ...
    style.textContent = `
      .cloud-auth-overlay {
        ...
        padding: 20px;
        background:
          radial-gradient(circle at top left, rgba(244,127,159,.18), transparent 38%),
          radial-gradient(circle at bottom right, rgba(114,199,162,.18), transparent 40%),
          rgba(255,250,247,.96);
      ...
      .cloud-auth-card {
        ...
        box-shadow: 0 28px 80px -34px rgba(100,70,80,.34);
      }
      .cloud-auth-logo {
        width: 58px;
        height: 58px;
        ...
        color: #e36f90;
        font-size: 25px;
        background: linear-gradient(145deg, #fff0e8, #ffe5ec);
      ...
      .cloud-field:focus {
        border-color: #F3A9BB;
        box-shadow: 0 0 0 4px rgba(244,127,159,.11);
      }
      .cloud-auth-actions {
        display: grid;
      ...
      .cloud-btn-primary {
        color: white;
        background: linear-gradient(90deg, #F47F9F, #F4A06F);
      }
      .cloud-btn-secondary {
        color: #5B484E;
        background: #FFF7F5;
      ...
      .cloud-status-badge {
        ...
        box-shadow: 0 12px 30px -22px rgba(100,70,80,.36);
      ...
      @media (max-width: 480px) {
        ...
        .cloud-status-badge { right: 12px; bottom: 12px; }
      }
    `;
    ...
    overlay.innerHTML = `
      <div class="cloud-auth-card">
        ...
        <p class="cloud-auth-subtitle">
          登录后，任务、论文、投稿、健康和复盘会在电脑与手机之间温柔同步。
        </p>
